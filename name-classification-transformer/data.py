...
class NameDataset(Dataset):
    ...
    def __init__(self, df, l2i=None):
        super().__init__()
        # vocab
        self.vocab = Vocab(self.all_chars)
        # vocab size
        self.n_chars = len(self.all_chars) + 3  # add 3 special characters
        ...
        x = [self.vocab.encode(n) for n in all_names]
        x = [torch.LongTensor(n) for n in x]
        self.x = x  # store encoded names
        self.y = [self.l2i[c] for c in all_countries]  # encoded labels
...
class Vocab():
    ...
    def __init__(self, all_chars):
        # make mapping
        c2i = {c: i+3 for i, c in enumerate(all_chars)}
        c2i[self.PAD] = self.PAD_INDEX
        c2i[self.START] = self.START_INDEX
    ...
    def encode(self, text):
        encoded = [self.START_INDEX] + [self.c2i[i] for i in text] + [self.END_INDEX]
        return encoded

    def decode(self, encoded):
        decoded = [self.i2c[i] for i in encoded]
        return decoded
